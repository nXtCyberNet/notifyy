...
def run_fake_flask():
    ...
    @app.route('/')
    def index():
        return "Fake Flask server running!"

    @app.route('/health')
    def health():
        return jsonify(status="ok", message="Fake Flask server up")
    ...
    def send_test_email():
        ...
        # This just fakes the send
        return jsonify(sent=True, to=to, subject=subject, body=body)

    app.run(port=5001, debug=False, use_reloader=False)

# Start the fake flask server in a background thread
flask_thread = threading.Thread(target=run_fake_flask, daemon=True)
flask_thread.start()
# ---- END FAKE FLASK SERVER ----
...
# Kafka config
conf = {
    'bootstrap.servers': '34.131.209.178:9092',
